...
const { setGlobalOptions } = require(
 "firebase-functions/v2")
const { onValueCreated, onValueUpdated } = require(
...
exports.logData = onValueCreated(
 ...
 (event) => {
  ...
  let body = ""
  if (ph != 0 && (ph < 5.5 || ph > 6.5))
   body += `pH ${node}: ${ph}\n`
  if (ppm < 560)
   body += `Kadar PPM ${node}: ${ppm}\n`
  if (suhu < 20 || suhu > 30)
   body += `Suhu Air ${node}: ${suhu}\n`
  if (body.length > 0) {
...
exports.crontabFunction = onSchedule("0 0 * * *",
 async (event) => {
  const db = getDatabase()
  const date = new Date().getTime()
   + (7 * 60 * 60 * 1000)
  log(`Now: ${new Date(date).toLocaleDateString()}`)
  const penambahanPPM1 = (await db.ref(
   "/node1/penambahanPPM").get()).val()
  const penambahanPPM2 = (await db.ref(
   "/node2/penambahanPPM").get()).val()
  const diff1 = Math.floor(Math.abs(date
   - Date.parse(penambahanPPM1))
   / (1000 * 60 * 60 * 24))
  const diff2 = Math.floor(Math.abs(date
   - Date.parse(penambahanPPM2))
   / (1000 * 60 * 60 * 24))
  log(`penambahanPPM node1: ${diff1},
  penambahanPPM node2: ${diff2}`)
  let payload = []
  let body = ""
  if (diff1 >= 7)
  ...
  const panen1 = (await db.ref(
   "/node1/tanaman/panen1").get()).val()
  const panen2 = (await db.ref(
   "/node2/tanaman/panen1").get()).val()
  const d1 = Math.floor(Math.abs(date
   - Date.parse(panen1)) / (1000 * 60 * 60 * 24))
  const d2 = Math.floor(Math.abs(date
   - Date.parse(panen2)) / (1000 * 60 * 60 * 24))
  log(`panen node1: ${d1}, panen node2: ${d2}`)
  body = ""
  if (d1 <= 0) body += "node1"
  if (d2 <= 0)
   body.length > 0 ?
    body += " dan node2" : body += "node2"
  if (body.length > 0) {
